...
survall <- lamecull %>%
  ...
  mutate(lifexinf = case_when(lifexlame == 0 ~ 0,
                                 lifexinf == 1 ~ 1,
                                 lifexinf == 2 ~ 2,
                                 lifexinf >2 ~3,
                                 TRUE ~ NA),
         lifexinf_cat = case_when(lifexlame == 0 ~ "Never any lesion",
                                     lifexinf == 1 ~ "Once",
                                     lifexinf == 2 ~ "Twice",
                                     lifexinf == 3 ~ "3 or more times",
                                     TRUE ~ NA) ) |> 
  mutate(lifexnoninf_cat = fct_reorder(lifexinf_cat, 
                                       lifexinf, .na_rm = TRUE))

fitKM <- survfit(Surv(censordat, culled) ~ lifexinf, data = survall)
km<- ggsurvplot(fitKM, 
                pval = FALSE, 
                facet.by = "farm",
                conf.int = TRUE,
